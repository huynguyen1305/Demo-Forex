...
const Hades = () => {
  ...
  useEffect(() => {
    const fetchDataHades = async () => {
      const response = await axios.get(
        "https://hadeswap-api.herokuapp.com/markets"
      );
      if (response) {
        const filterList = response.data.map((item: any, idx: number) => ({
          no: idx + 1,
          listings: item.listingsAmount,
          collectionImage: item.collectionImage,
          collectionName: item.collectionName,
          floorPrice:
            Number(item.floorPrice?.replace(".", "")) || item.floorPrice,
          bestOffer: Number(item.bestoffer?.replace(".", "")) || item.bestoffer,
          offerTvl: Number(item.offerTVL?.replace(".", "")) || item.offerTVL,
          marketPubkey: item.marketPubkey,
        }));
